...
void OLED_WR_Byte(u8 dat,u8 cmd)//SPI放这里
{	
	u8 i;			  
	if(cmd)
	...
	for(i=0;i<8;i++)
	{			  
		digitalWrite(OLED_SCL, 0); //OLED_SCL_Clr();
		if(dat&0x80)
		{
			digitalWrite(OLED_SDA, 1); //OLED_SDA_Set();
		}
		else
		{
			digitalWrite(OLED_SDA, 0); //OLED_SDA_Clr();
		}
		digitalWrite(OLED_SCL, 1); //OLED_SCL_Set();
		dat<<=1;   
	}				 		  
